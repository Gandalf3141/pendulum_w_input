...
def plot_data(data, time, plot_derive=0, plot_all=True):
    ...
    if not plot_all:
        for i in range(data.size(dim=0)):
            # , label="example_training_data")
            plt.plot(time, data[i, :, plot_derive].detach().numpy())
    ...
    if plot_all:
        for i in range(data.size(dim=0)):
            ...
            axs[1].plot(time, data[i, :, 1], color="red",
                        label=f'{greek_letterz[7]}(t)')
            ...
            axs[2].plot(time,  data[i, :, 2], color="blue",
                        label=f'{greek_letterz[24]}(t)')
...
def plot_batch(batch_y0, batch_t, batch_y):
    for i in range(batch_y.size(dim=1)):
        plt.plot(batch_t, batch_y[:, i, 0].detach(
        ).numpy(), label="batch for one trajectory")
    plt.grid()
    plt.legend()
    plt.show()
...
def plot_batch_rnn(data, batch_y0_rnn, batch_t_rnn, batch_y_rnn, t_end_batch, t_start, time_steps):
   # plt.plot(time, data[0,:,0].detach().numpy(), label="example_training_data", color="grey")
    for i in range(batch_y_rnn.size(dim=1)):
        plt.scatter(time[i*t_end_batch: (i+1)*t_end_batch], batch_y_rnn[:, i, 0].detach(
        ).numpy(), label=f"{i} batch_rnn for one trajectory", marker="o", s=25)
    plt.grid()
    plt.plot(time, data[0, :, 0].detach().numpy(),
             label="example_training_data", color="grey")
    plt.legend()
    plt.show()
